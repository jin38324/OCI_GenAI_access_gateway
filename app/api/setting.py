...
exponential_backoff_retry_strategy = RetryStrategyBuilder(
    ...
    should_retry_on=lambda response: response.status
    in [429, 500, 502, 503, 504],  # Retry on throttling/server errors
).get_retry_strategy()
...
if AUTH_TYPE == "API_KEY":
    ...
    OCI_CONFIG_FILE_KEY = os.environ.get(
        "OCI_CONFIG_FILE_KEY", config.OCI_CONFIG_FILE_KEY
    )
...
INFERENCE_ENDPOINT_TEMPLATE = (
    "https://inference.generativeai.{region}.oci.oraclecloud.com/20231130"
)
...
if OCI_REGION and OCI_COMPARTMENT:
    ...
    SUPPORTED_OCIGENAI_EMBEDDING_MODELS = get_supported_models(
        MODEL_SETTINGS, "embedding"
    )
    SUPPORTED_OCIGENAI_CHAT_MODELS = get_supported_models(
        MODEL_SETTINGS, "ondemand"
    ) | get_supported_models(MODEL_SETTINGS, "dedicated")
