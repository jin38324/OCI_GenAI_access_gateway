...
class ChatRequestAdapter:
    ...
    def to_oci(self, chat_request: ChatRequest) -> oci_models.ChatDetails:
        ...
        if self.provider == "cohere":
            return ChatRequestAdapter.CohereAdapter.to_cohere(
                chat_request, servingMode, compartment_id
            )
        else:
            return ChatRequestAdapter.GenericAdapter.to_generic(
                chat_request, servingMode, compartment_id
            )
    ...
    class GenericAdapter:
        ...
        def to_generic(chat_request: ChatRequest, servingMode, compartment_id):
            ...
            for (
                key,
                value,
            ) in ChatRequestAdapter.GenericAdapter.CHAT_REQUEST_MAPPER_DIFF.items():
            ...
            if chat_request.stream_options:
                is_include_usage = chat_request.stream_options.get(
                    "include_usage", True
                )
            ...
            if chat_request.prediction:
                oci_chat_request.prediction = oci_models.StaticContent(
                    content=MessageAdapter.ContentAdapter.to_generic_content(
                        chat_request.prediction.content
                    )
                )

            # Tool choice / reasoning effort / verbosity
            if chat_request.tool_choice:
                oci_chat_request.tool_choice = (
                    ChatRequestAdapter.GenericAdapter.TOOL_CHOICE_MAPPER.get(
                        chat_request.tool_choice
                    )
                )
            if chat_request.reasoning_effort:
                oci_chat_request.reasoning_effort = (
                    chat_request.reasoning_effort.upper()
                )
            ...
            if chat_request.response_format:
                oci_chat_request.response_format = ResponseFormatAdapter.to_generic(
            ...
            if chat_request.tools:
                oci_chat_request.tools = ToolAdapter.ToolsDefinitionAdapter.to_generic(
                    chat_request.tools
                )
    ...
    class CohereAdapter:
        ...
        def to_cohere(chat_request: ChatRequest, servingMode, compartment_id):
            ...
            for (
                key,
                value,
            ) in ChatRequestAdapter.CohereAdapter.CHAT_REQUEST_MAPPER_DIFF.items():
                inference_config[value] = getattr(chat_request, key)
            ...
            if chat_request.response_format:
                oci_chat_request.response_format = ResponseFormatAdapter.to_cohere(
                    chat_request.response_format
                )
            ...
            chat_history, cohere_message, cohere_tool_results, preamble_override = (
                MessageAdapter.to_cohere(chat_request.messages)
            )
            ...
            if chat_request.tools:
                oci_chat_request.tools = ToolAdapter.ToolsDefinitionAdapter.to_cohere(
                    chat_request.tools
                )
...
class MessageAdapter:
        ...
        for msg in messages:
            ...
            # AssistantMessage
                ...
                if msg.get("tool_calls"):
                    new_msg.tool_calls = ToolAdapter.ToolCallAdapter.to_generic(
                        msg["tool_calls"]
                    )
                elif content:
            ...
            # ToolMessage
                ...
                new_msg = ToolAdapter.ToolResultAdapter.to_generic(
                    content, msg.get("tool_call_id")
                )
    ...
    def to_cohere(
        messages: List[ChatCompletionMessageParam],
    ) -> list[oci_models.CohereMessage]:
        """
        Convert OpenAI messages to Cohere messages.
        """
        chatHistory, cohere_message, cohere_tool_results, preamble_override = (
            None,
            "",
            None,
            None,
                ...
                if message["role"] == "system":
                    ...
                    if message["role"] == "user":
                        ...
                        if message.get("tool_calls"):
                            new_tool_calls, tool_info = (
                                ToolAdapter.ToolCallAdapter.to_cohere(
                                    message["tool_calls"]
                                )
                            )
                            openai_tool_calls.update(tool_info)
                            new_msg = oci_models.CohereChatBotMessage(
                                message=text, tool_calls=new_tool_calls
                            )
                        else:
                            new_msg = oci_models.CohereChatBotMessage(message=text)

                    elif message["role"] == "tool":
                        new_tool_results = ToolAdapter.ToolResultAdapter.to_cohere(
                            openai_tool_calls, message
                        )
        ...
        if last_message["role"] in ("user", "assistant"):
            cohere_message = MessageAdapter.ContentAdapter.to_str(
                last_message["content"]
            )

        elif last_message["role"] == "tool":
            cohere_tool_results = ToolAdapter.ToolResultAdapter.to_cohere(
                last_message.get("tool_call_id"),
                openai_tool_calls,
                last_message["content"],
            )
    ...
    class ContentAdapter:
        @staticmethod
        def to_generic_content(
            content: Union[str, List],
        ) -> list[oci_models.ChatContent]:
            """
            ...
            if isinstance(content, str):
                ...
                for each in content:
                    if each["type"] == "text":
                        ...
                        contents.append(
                            oci_models.ImageContent(image_url=each["image_url"])
                        )
                    else:
