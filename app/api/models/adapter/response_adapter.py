...
class ResponseAdapter:
    ...
    def to_openai(
        ...
        message_id: str,
        model: str,
        response: oci_models.GenericChatResponse | oci_models.CohereChatResponse,
    ) -> ChatCompletion:
        openai_choices = []
            ...
            for choice in response.choices:
                openai_choices.append(
                    Choice(
                        ...
                        finish_reason=FinishReasonAdapter.to_openai(
    ...
    def to_openai_chunk(
        self, message_id: str, model: str, chunk: dict
    ) -> ChatCompletionChunk:
        ...
        return ChatCompletionChunk(
            ...
            choices=[
                ChoiceChunk(
                    index=index, logprobs=None, finish_reason=finish_reason, delta=delta
                )
            ],
            usage=usage,
            system_fingerprint=None,
...
class MessageAdapter:
    ...
    def from_generic(message: oci_models.Message) -> ChatCompletionMessage:
        ...
        if message.tool_calls:
            ...
            chat_completion_message.tool_calls = ToolAdapter.ToolCallAdapter.to_openai(
                message.tool_calls
            )
    ...
    def from_chunk_message(chunk: dict) -> ChoiceDelta:
        choice_delta = ChoiceDelta(role="assistant", refusal="")
...
class UsageAdapter:
    ...
    def from_chunk_usage(usage_dict: dict | None) -> CompletionUsage | None:
        ...
        if usage_dict.get("completionTokensDetails"):
            completion_tokens_details = CompletionTokensDetails(
                accepted_prediction_tokens=usage_dict["completionTokensDetails"].get(
                    "acceptedPredictionTokens"
                ),
                reasoning_tokens=usage_dict["completionTokensDetails"].get(
                    "reasoningTokens"
                ),
